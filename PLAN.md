...
- **외부 API 통합**: `docs/API_INTEGRATION_GUIDE.md`를 기반으로 매크로(ECOS/FRED) 및 뉴스(네이버) 데이터 연동 시작.
- **AI 분석 레이어**: 수집된 4단계 데이터(Macro -> Industry -> Fundamental -> Valuation/News)를 AI 프롬프트에 동적으로 주입하는 구조 설계.
- **NXT(넥스트레이드) 실시간 시세 연동**: 거래시간 확장(08:00~20:00) 대응 및 신규 실시간 체결 타입 구독 로직 확대. (상세 계획: [docs/nxt_integration_guide.md](./docs/nxt_integration_guide.md) 참조)
