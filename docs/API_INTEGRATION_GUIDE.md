...
### [4단계] 밸류에이션 및 매크로 / 뉴스 (Valuation, Macro & Sentiment)
*   **목적:** "과거 10년 밴드 대비 현재 주가의 위치는 어디이며, 매크로 시황과 최근 반응은 어떠한가?" 판단
*   **데이터 소스:**
    *   **Yahoo Finance (`yahoo-finance2`):** 
        *   **원칙:** "과거 10년치 장기 월봉 주가 히스토리" 및 "글로벌 매크로 지표(환율, 미국채, S&P500 등)" 용도로만 한정. 
        *   **격리:** 로컬 시세(키움) 및 기업 재무정보(DART)와는 철저히 격리하며 중복 데이터는 수집 금지.
    *   **네이버 뉴스 검색 API:** 종목별 최신 기사 및 속보 수집
*   **AI 활용:** 
    *   역사적 10년 밸류에이션 하단 진입 여부 판단 (P/E, P/B Band 등 모델링 보조)
    *   뉴스 텍스트 분석을 통한 긍정/부정 감성 점수 (Sentiment Score) 부여
    *   거시 경제 환경(환율 등)이 특정 종목 산업군에 미칠 영향 분석
