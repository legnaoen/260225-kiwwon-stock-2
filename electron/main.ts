...
ipcMain.handle('v2:resume-post-market-pipeline', async () => {
    try {
        ...
        // 2. 15:35 Live Trade Reconciliation & Moonshot Tracker
        console.log('[Manual Resume] 2. Live Trade Recon & Moonshot Tracker (15:35)');
        try {
            const { LiveTradeReconciliationService } = await import('./services/LiveTradeReconciliationService');
            await LiveTradeReconciliationService.getInstance().reconcileDailyExecutions();
        } catch (e: any) {
            console.error('[Manual Resume] LiveTradeReconciliationService 오류:', e);
        }
    ...
    try {
        ...
        for (const strategy of activeLiveStrategies) {
            ...
            for (const pick of todayPicks) {
                ...
                let currentPrice = ohlcvRow?.close ?? 0;

                if (currentPrice <= 0) {
                    try {
                        const { KiwoomService } = await import('./services/KiwoomService');
                        const priceInfo = await KiwoomService.getInstance().getStockBasicInfo(pick.stock_code);
                        const body = priceInfo?.Body || priceInfo?.out1 || priceInfo || {};
                        const rawCur = String(body.cur_prc || body.stk_prc || body.stck_prpr || '0').replace(/[^0-9-]/g, '');
                        currentPrice = Math.abs(parseInt(rawCur, 10)) || 0;
                    } catch (err: any) {
                        console.error(`[Manual Buy] ${pick.stock_code} 현재가 대체 수집 실패:`, err.message);
                    }
                }
