...
텐베거(10-Bagger) 포트폴리오의 생명은 '희소성'과 '확실성'입니다. 무분별한 편입은 장기 수익률을 심각하게 훼손합니다.
아래 ${categoryTag} 전용 심사 기준을 통해 서로 가장 가혹하게 **상대 평가(배틀로얄)**하여, '초격차 1위(가장 상승 모멘텀이 확실한 단 1개 종목)'만 isPass: true 로 가려내십시오.
1위 종목이라도 텐베거의 폭발력이 부족하다면 주저 없이 전원 탈락(isPass: false) 시키십시오.
(tbpScore는 0~100점 만점이며, 80점 미만은 무조건 탈락입니다.)
