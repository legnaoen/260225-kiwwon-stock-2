        ...
        for (const c of candidates) {
            ...
            if (ohlcv) {
                ...
                if (ohlcv.change_rate > MAX_OVERHEAT_CHANGE_RATE) {
                    console.log(`[TrackBBuyAgent] 당일 과열(+${MAX_OVERHEAT_CHANGE_RATE}% 초과) 선별 제외: ${c.stockName}(${c.stockCode}) +${ohlcv.change_rate.toFixed(1)}%`);
                    skipped.push(c);
                    continue;
                }
            }
...
위 정보를 바탕으로 이 종목의 20영업일(약 한 달) 내 +15% 달성 가능성을 분석하십시오.`;
        ...
        if (isBypass) {
            ...
            import('../TelegramService').then(m => m.TelegramService.getInstance().sendMessage(`[TrackB] ☁️ 클라우드 쾌속 분석 모드 (병렬 ${candidates.length}개) 작동 중...`));
    ...
    private async runAiAnalysis(
        ...
        let phase2Guideline = '';
        try {
            const guidelinePath = path.join(process.cwd(), 'guidelines', `track_${'B'.toLowerCase()}_phase2.md`);
            if (fs.existsSync(guidelinePath)) {
...
최종 Top ${targetPicks}개를 BUY로 선정하고, 나머지는 WATCH로 처리하십시오.`;
            ...
            try {
                ...
                if (!result) {
                    console.error(`[TrackBBuyAgent] Phase4 AI 응답 없음 (그룹 ${chunkIndex + 1})`);
                    return;
                }

                const jsonMatch = result.match(/\[[\s\S]*\]/);
                if (!jsonMatch) {
                    console.error(`[TrackBBuyAgent] Phase4 JSON 추출 실패 (그룹 ${chunkIndex + 1}):`, result.slice(0, 200));
                    return;
                }
