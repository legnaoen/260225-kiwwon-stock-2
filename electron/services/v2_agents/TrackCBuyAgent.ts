...
위 정보를 바탕으로 이 종목의 10영업일(약 2주) 내 +15% 달성 가능성을 분석하십시오.`;
    ...
    private async runAiAnalysis(
        ...
        console.log(`[TrackCBuyAgent] ⚖️ 동적 컷오프 적용: 리스크 ${riskScore}점 -> ${cutoffConfig.description}`);
        ...
        console.log(`[TrackCBuyAgent] Phase4 대상: ${eligibleCandidates.length}개 (Gemma 1차 컷오프 ${cutoffConfig.gemmaMinScore}점 미만 제외)`);
...
최종 Top ${targetPicks}개를 BUY로 선정하고, 나머지는 WATCH로 처리하십시오.`;
        ...
        const filteredPicks = allPicks.map(p => {
            if (p.decision === 'BUY' && p.buy_score < cutoffConfig.geminiPassScore) {
                console.log(`[TrackCBuyAgent] 🚨 AI 오작동 필터링: ${p.stock_name}(${p.stock_code}) 점수 ${p.buy_score}점이 합격선 ${cutoffConfig.geminiPassScore}점 미만이므로 WATCH로 격하 처리.`);
