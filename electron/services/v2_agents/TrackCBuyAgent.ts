...
위 정보를 바탕으로 이 종목의 10영업일(약 2주) 내 +15% 달성 가능성을 분석하십시오.`;
        ...
        if (isBypass) {
            ...
            this.telegram.sendMessage(`[TrackC] ☁️ 클라우드 쾌속 분석 모드 (병렬 ${candidates.length}개) 작동 중...`);
   ...
   근거: ${r.reasoning}`;
        ...
        let phase2Guideline = '';
        try {
            const guidelinePath = path.join(process.cwd(), 'guidelines', 'track_c_phase2.md');
            if (fs.existsSync(guidelinePath)) {
...
[분석 대상: ${eligibleCandidates.length}개 눌림목 스나이핑 후보 종목]
...
최종 Top ${targetPicks}개를 BUY로 선정하고, 나머지는 WATCH로 처리하십시오.`;
        ...
        try {
            ...
            if (!result) {
                console.error('[TrackCBuyAgent] Phase4 AI 응답 없음');
                return [];
            }

            const jsonMatch = result.match(/\[[\s\S]*\]/);
            if (!jsonMatch) {
                console.error('[TrackCBuyAgent] Phase4 JSON 추출 실패:', result.slice(0, 200));
                return [];
            }

            const picks: AiBuyPick[] = JSON.parse(jsonMatch[0]);
            console.log(`[TrackCBuyAgent] Phase4 결과: BUY ${picks.filter(p => p.decision === 'BUY').length}개, WATCH ${picks.filter(p => p.decision === 'WATCH').length}개`);
            return picks;

        } catch (err: any) {
            console.error('[TrackCBuyAgent] Phase4 AI 분석 오류:', err.message);
            return [];
        }
    }
