...
[응답 형식] 반드시 다음 JSON 객체만 출력하십시오. 설명 없이 JSON만:
{
  ...
  "reasoning": "이 종목을 내일 종가베팅 대상에 포함해야 하는가? (1줄 요약)"
        ...
        const isBypass = Array.isArray(aiSettings.lightweightCloudAgents) && aiSettings.lightweightCloudAgents.some((id: string) => 'TRACK_D_GEMMA_RESEARCH'.includes(id) || 'TRACK_D_GEMMA_RESEARCH'.startsWith(id));
...
위 정보를 바탕으로 이 종목의 10영업일 내 +15% 달성 가능성을 분석하십시오.`;
            ...
            try {
                const rawResponse = await AiExecutionQueue.getInstance().enqueue({
                    agentId: 'TRACK_D_GEMMA_RESEARCH',
        ...
        if (isBypass) {
            ...
            import('../TelegramService').then(m => m.TelegramService.getInstance().sendMessage(`[TrackD] ☁️ 클라우드 쾌속 분석 모드 (병렬 ${candidates.length}개) 작동 중...`));
    ...
    private async runAiAnalysis(
        ...
        console.log(`[TrackDBuyAgent] ⚖️ 동적 컷오프 적용: 리스크 ${riskScore}점 -> ${cutoffConfig.description}`);
        ...
        console.log(`[TrackDBuyAgent] Phase4 대상: ${eligibleCandidates.length}개 (Gemma 1차 컷오프 ${cutoffConfig.gemmaMinScore}점 미만 제외)`);
        ...
        let phase2Guideline = '';
        try {
            const guidelinePath = path.join(process.cwd(), 'guidelines', `track_${'D'.toLowerCase()}_phase2.md`);
            if (fs.existsSync(guidelinePath)) {
                phase2Guideline = fs.readFileSync(guidelinePath, 'utf-8');
            }
        } catch (e) {
            console.warn(`[TrackD] Failed to load phase2 guideline:`, e);
        }
...
최종 Top ${TARGET_PICKS}개를 BUY로 선정하고, 나머지는 WATCH로 처리하십시오.`;
            ...
            try {
                ...
                if (!result) {
                    console.error(`[TrackDBuyAgent] Phase4 AI 응답 없음 (그룹 ${chunkIndex + 1})`);
                    return;
                }

                const jsonMatch = result.match(/\[[\s\S]*\]/);
                if (!jsonMatch) {
                    console.error(`[TrackDBuyAgent] Phase4 JSON 추출 실패 (그룹 ${chunkIndex + 1}):`, result.slice(0, 200));
                    return;
                }
                ...
                console.error(`[TrackDBuyAgent] Phase4 AI 분석 오류 (그룹 ${chunkIndex + 1}):`, err.message);
            }
        }));

        console.log(`[TrackDBuyAgent] Phase4 통합 결과: BUY ${allPicks.filter(p => p.decision === 'BUY').length}개, WATCH ${allPicks.filter(p => p.decision === 'WATCH').length}개`);
        ...
        const filteredPicks = allPicks.map(p => {
            if (p.decision === 'BUY' && p.buy_score < cutoffConfig.geminiPassScore) {
                console.log(`[TrackDBuyAgent] 🚨 AI 오작동 필터링: ${p.stock_name}(${p.stock_code}) 점수 ${p.buy_score}점이 합격선 ${cutoffConfig.geminiPassScore}점 미만이므로 WATCH로 격하 처리.`);
